...
import { Form, Button, Input, Card, message, Col, Radio, Select } from 'antd';
...
class MediaListModify extends Component {
  constructor(props) {
    super(props);
    this.state = {
      initMpData: {
  ...
  initial = () => {
    ...
    ajax({
      url: `/api/v1/mp?mpName=${this.props.location.state.name}`,
      method: 'GET'
    })
      .then((res) => {
        console.log('res: ', res, res.data.list[0].category, categoryData['工具'], categoryData[res.data.list[0].category]);
        // 定点处理脏数据...
        const subCategory = categoryData[res.data.list[0].category] || this.state.subCategory;
        this.setState({
          category: res.data.list[0].category,
          initMpData: res.data.list[0],
          subCategory,
          subCategoryValue: res.data.list[0].subCategory
        });
        this.props.form.resetFields(['category', 'subCategory']);
        console.log('this.state.initMpData.type.code: ', this.state.initMpData.type.code);
      })
      .catch((error) => {
        ...
        ajax({
          ...
          .then((res) => {
            ...
            if (res.code === 0) {
              ...
              // window.location.href = '/project/mediamgr/medialist';
  ...
  handleCategoryChange = (value) => {
    ...
    this.setState({
      ...
      subCategoryValue: categoryData[value][0]
    });
    this.props.form.resetFields('subCategory', categoryData[value]);
  };
  ...
  onRadioChange = (e) => {
    ...
    if (e.target.value === 'miniGame') {
      this.setState({
        ...
        subCategoryValue: categoryData['游戏'][0]
      });
    } else {
      this.setState({
        category: this.state.initMpData.category,
        subCategory: categoryData[this.state.initMpData.category],
        subCategoryValue: this.state.initMpData.subCategory
      });
    }
  ...
  render() {
    ...
    console.log(
      ...
      serverCategoryData[0],
      categoryData[serverCategoryData[0]]
    );
          ...
          <Form onSubmit={this.onSubmit} style={{ maxWidth: 'initial' }}>
            <Form.Item {...formItemLayout} label="小程序名称">
              {getFieldDecorator('appName', {
                ...
                initialValue: this.state.initMpData.appName
            ...
            <Form.Item label="小程序类型" {...formItemLayout}>
              {getFieldDecorator('mpType', {
                ...
                initialValue: this.state.initMpData.type.code === 0 ? 'miniProgram' : 'miniGame'
              })(
            ...
            <Form.Item label="APPID" {...formItemLayout}>
              {getFieldDecorator('appId', {
                ...
                initialValue: this.state.initMpData.appId
              ...
              <Col span={11}>
                <Form.Item>
                  {getFieldDecorator('subCategory', {
                    ...
                    initialValue: this.state.subCategoryValue
                  })(
                    <Select
            ...
            <Form.Item label="小程序描述" style={{ marginTop: 30 }} {...formItemLayout}>
              {getFieldDecorator('description', {
                initialValue:
                  this.state.initMpData.description === ''
                    ? '请输入描述内容'
                    : this.state.initMpData.description
...
export default Form.create()(MediaListModify);
